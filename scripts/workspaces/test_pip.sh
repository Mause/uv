...
(
  ...
  uv venv && \
  cargo run --profile fast-build -- pip install --preview --offline --find-links "$(git rev-parse --show-toplevel)/scripts/workspaces/vendored" -e . -r pyproject.toml && \
  .venv/bin/python check_installed_bird_feeder.py
)
(
  cd "$(git rev-parse --show-toplevel)/scripts/workspaces/albatross-in-example" && \
  uv venv && \
  cargo run --profile fast-build -- pip install --preview --offline --find-links "$(git rev-parse --show-toplevel)/scripts/workspaces/vendored" -e . -r pyproject.toml && \
  .venv/bin/python check_installed_albatross.py
)
(
