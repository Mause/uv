    ...
    python_executables(version, sources).map(|result| match result {
        Ok((source, path)) => Interpreter::query(&path, cache)
            ...
            .inspect(|(source, interpreter)| {
                trace!(
                    "Discovered Python interpreter {} {} at {} from {source}",
...
mod tests {
    ...
    use crate::{
        ...
        implementation::ImplementationName,
        InterpreterNotFound, InterpreterSource, PythonVersion, SourceSelector,
    };
    ...
    fn create_mock_python2_interpreter(path: &Path) -> Result<()> {
        // Python 2 does not support the `-I` option we use when querying interpreters.
        // Emulate its error for testing
        let output = indoc! { r"
            Unknown option: -I
    ...
    fn create_children(tempdir: &TempDir, names: &[OsString]) -> Result<Vec<PathBuf>> {
    ...
    fn find_default_interpreter_empty_path() -> Result<()> {
        ...
        with_vars([("UV_TEST_PYTHON_PATH", None), ("PATH", Some(""))], || {
        ...
        with_vars(
            [("UV_TEST_PYTHON_PATH", None), ("PATH", None::<OsString>)],
            || {
                let result = find_default_interpreter(&cache);
                assert!(
                    matches!(
    ...
    fn find_default_interpreter_valid_executable_after_invalid() -> Result<()> {
        let tempdir = TempDir::new()?;
        let cache = Cache::temp()?;
        tempdir.child("query-error").create_dir_all()?;
        tempdir.child("not-executable").create_dir_all()?;
        ...
        // An empty executable file, should fail to parse response
        tempdir
            .child("query-error")
            .child(format!("python{}", std::env::consts::EXE_SUFFIX))
            .touch()?;
            ...
            [
                ...
                (
                    ...
                    Some(env::join_paths([
                        ...
                        tempdir.child("not-executable").as_os_str(),
                        tempdir.child("query-error").as_os_str(),
